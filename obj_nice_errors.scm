;;; obj.scm version 4.1 10/31/2012
...
;; Revised to throw nicer errors by Pierre Karashchuk
...
(define-macro (define-class . body) (check-and-make-definitions body))

(define (check-and-make-definitions form)
  (if (check form)
      (make-definitions form)
      (error "invalid form") ; really, check should throw its own error
      ))
...
(define (check form)
  (check-parent form)
  (check-class-vars form)
  (check-instance-vars form)
  ;;  (check-init form) ; I can't think of a way initialize could crash
  (check-methods form)
  #t)

(define (check-instance-vars form)
  (let ((instance-vars-clause (find-a-clause 'instance-vars form)))
    (for-each (lambda (decl)
                (unless (and (pair? decl) (symbol? (car decl))
                             (pair? (cdr decl)))
                        (error "Invalid instance variable declaration: ~A~%in ~A~%Must be of the form (<variable> <value>)~%" decl instance-vars-clause)))
              (if (null? instance-vars-clause) '()
                  (cdr instance-vars-clause)))))

(define (check-parent form)
  (let ((parent-clause (find-a-clause 'check-parent form)))
    (for-each (lambda (p)
                (unless (pair? p)
                        (error "Invalid parent:~A~%in ~A%~Must be of the form (<parent-name> <arg1> <arg2> ...)~%" p parent-clause)))
              (if (null? parent-clause) '()
                  (cdr parent-clause)))))

(define (check-class-vars form)
  (let ((vars-clause (find-a-clause 'class-vars form)))
    (for-each (lambda (decl)
                (unless (and (pair? decl) (symbol? (car decl))
                             (pair? (cdr decl)))
                        (error "Invalid class variable declaration: ~A~%in ~A~%Must be of the form (<variable> <value>)~%" decl vars-clause)))
              (if (null? vars-clause) '()
                  (cdr vars-clause)))))

(define (check-methods form)
  (for-each
   (lambda (method-defn)
     (unless (and (>= (length method-defn) 3)
                  (>= (length (cadr method-defn)) 1))
             (error "Invalid method: ~A~%Must be of the form (method (<name> <arg1> <arg2> ...) <body>)~%" (pp method-defn #f))))
   (obj-filter (cdr form) (lambda (x) (eq? (car x) 'method))) ))
